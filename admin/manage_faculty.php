		...
		<div class = "container-fluid" style = "margin-top:70px;">
				...
				<div class = "modal-dialog" role = "document">
					<div class = "modal-content panel-primary">
						<div class = "modal-header panel-heading">
							...
							<h4 class = "modal-title" id = "myModallabel">Manage Faculty</h4>
						</div>
                        ...
                        <div class  = "modal-body">
                            ...
                            <div class = "form-group">
									...
									<input type = "text" name = "class_count" required = "required" class = "form-control" />
								</div>
                        </div>
			...
			<div class = "well col-lg-12">
				...
				<table id = "table" class = "table table-bordered">
					...
					<tbody>
						<?php
							$q_manage_faculty = $conn->query("SELECT * FROM `manage_faculty`") or die(mysqli_error());
							while($f_manage_faculty = $q_manage_faculty->fetch_array()){
                                $q_manage_faculty_subject_name = $conn->query("SELECT subject_name FROM `subjects` WHERE `subject_id` = '$f_manage_faculty[subject_id]'") or die(mysqli_error());
                                $f_manage_faculty_subject_name = $q_manage_faculty_subject_name->fetch_array();

                                $q_manage_faculty_faculty_name = $conn->query("SELECT faculty_name FROM `faculty` WHERE `faculty_id` = '$f_manage_faculty[faculty_id]'") or die(mysqli_error());
                                $f_manage_faculty_faculty_name = $q_manage_faculty_faculty_name->fetch_array();
                        ?>
						<tr>
							<td><?php echo $f_manage_faculty['course_id']?></td>
							<td><?php echo $f_manage_faculty_subject_name['subject_name']?></td>
							<td><?php echo $f_manage_faculty_faculty_name['faculty_name']?></td>
							<td><?php echo $f_manage_faculty['class_count']?></td>
							<td><a class = "btn btn-danger rfaculty_id" name = "<?php echo $f_manage_faculty['manage_faculty_id']?>" href = "#" data-toggle = "modal" data-target = "#delete"><span class = "glyphicon glyphicon-remove"></span></a> <a class = "btn btn-warning  efaculty_id" name = "<?php echo $f_manage_faculty['manage_faculty_id']?>" href = "#" data-toggle = "modal" data-target = "#edit_faculty"><span class = "glyphicon glyphicon-edit"></span></a></td>
						</tr>
						<?php
							}
						?>
	...
	<script type = "text/javascript">
		$(document).ready(function(){
			$('.rfaculty_id').click(function(){
				manage_faculty_id = $(this).attr('name');
				$('.remove_id').click(function(){
					window.location = 'delete_manage_faculty.php?manage_faculty_id=' + manage_faculty_id;
				});
			});
			$('.efaculty_id').click(function(){
				manage_faculty_id = $(this).attr('name');
				$('#edit_query').load('load_edit_manage_faculty.php?manage_faculty_id=' + manage_faculty_id);
			});
		});
	</script>
</html>